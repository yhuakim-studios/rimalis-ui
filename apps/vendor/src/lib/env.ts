 ...
 * API's local `corsOrigins` allows `localhost:5175`, and then fail completely in
 * production, where `corsOrigins` is `[]`.
 *
 * That failure mode is worse here than on the storefront. Every vendor call is
 * authenticated, so a browser-side fetch would need the access token in the
 * browser — which is the one thing the BFF exists to prevent.
 *
...
export function sessionSecret(): string {
  ...
  if (value === undefined) {
    throw new Error(
      "SESSION_SECRET is not set in this environment. On Cloudflare, set it with `wrangler secret put SESSION_SECRET` from apps/vendor; locally, add it to apps/vendor/.env.local.",
    );
  }
  return value;
}
