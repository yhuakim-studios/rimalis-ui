 ...
 * server and the tokens never reach this bundle — see `auth-actions.ts`.
 */
...
const REASONS: Record<string, string> = {
  ...
  // vendor has just been signed out on every device and deserves to know it was
  // a security measure rather than a fault.
  revoked:
    "We signed you out of every device as a precaution. Sign in again — nothing on your store has changed.",
    ...
    <form action={formAction} className="flex flex-col gap-5" noValidate>
      ...
      */}
      ...
      <p className="text-caption text-ink-muted">
        Selling on Rimalis starts with an ordinary Rimalis account — sign up on the storefront
        first, then apply here.
      </p>
    </form>
  );
}
