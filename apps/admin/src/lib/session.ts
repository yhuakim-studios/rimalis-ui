 ...
 * untampered-with.
 *
 * ## `SameSite=Strict`, unlike the other two apps
 *
 * The marketplace and vendor apps both had to settle for `Lax` because Paystack
 * returns the user to them by a **cross-site top-level GET**, and a Strict cookie
 * is not sent on one — the shopper or vendor would land on a login page
 * immediately after being charged.
 *
 * This app has no such hop. Nothing off-site links into the admin console: no
 * payment return, no email deep link, no public page. Every entry is somebody
 * typing the URL or following an internal link, and every write is a Server
 * Action (POST) behind Next's action-id check. So it can afford the strictest
 * setting, and it is the app where that is worth most — this session grants
 * ADMIN.
 *
 * ⚠️ THE TRIGGER TO CHANGE THIS: the first off-site link INTO the console. An
 * emailed "a vendor is awaiting approval — review it here" is exactly that, and
 * the click originates off-site. On the day that ships, this becomes `Lax` or the
 * link lands on a login screen. Do not discover it in production.
 *
 * ## The cookie NAME is load-bearing in development
 *
 * `rimalis_admin_session`, not `rimalis_session`. In production the three apps
 ...
 * `localhost:5173` (marketplace), `:5174` (admin) and `:5175` (vendor) share one
 * cookie jar.
 *
 * With a shared name, signing into the admin console overwrites the vendor's
 * cookie and silently signs them out, and vice versa. All three apps also use
 * different `SESSION_SECRET`s, so each would receive a cookie it cannot decrypt
 * and read it as "not signed in" — a session that appears to drop at random
 * depending on which app was opened last. Neither symptom points at the cookie
 * name.
 *
 ...
 * identically until someone "fixes" it to `.example.com`, at which point the
 * storefront and the vendor app can read the ADMIN session — every shopper's
 * browser holding a credential that can suspend a vendor. The three-subdomain
 * separation ADR-0003 exists to provide would be silently gone. An attribute
 * that is absent cannot be mistyped.
 */

export const SESSION_COOKIE = "rimalis_admin_session";
 ...
 * produces an admin the app believes is signed in, whose every request 401s.
 */
 ...
 *
 * The user summary is denormalised in on purpose: rendering the signed-in
 * admin's email in the header must not cost an API call on every request, and the fields it
 ...
 * it is what every admin holds the moment `SESSION_SECRET` is rotated, and
...
export const sessionCookieOptions = () =>
  ({
    httpOnly: true,
    // Strict, unlike the other two apps — see the module header. Nothing off-site
    // links into the console, so there is no cross-site top-level GET to
    // accommodate. The first emailed deep link into here changes this to "lax".
