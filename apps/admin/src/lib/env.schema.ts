 ...
 *
 * ## The narrowest of the three copies
 *
 * No `APP_ORIGIN`: this app has no OpenGraph cards, no public pages, and — unlike
 * the vendor app — no off-site return hop at all. Nothing outside it ever links
 * in, so there is no absolute URL for it to know about itself.
 *
 * No `MARKETPLACE_FIXTURES`: there is no public catalogue here.
 */
 ...
 *
 * ⚠️ Use a THIRD secret, different from both the marketplace's and the vendor
 * app's. Sharing one would let a cookie minted by one app decrypt in another,
 * which is precisely the cross-app session bleed that three separate subdomains
 * exist to prevent — and it matters most here, because this is the app whose
 * session grants ADMIN.
 */
...
function parse<T extends z.ZodType>(
  ...
  throw new Error(
    ["", "Invalid environment for @rimalis/admin:", "", ...lines, "", ...hint, ""].join("\n"),
  );
}
...
export function parseEnv(source: Record<string, string | undefined>): Env {
  return parse(envSchema, source, [
    "Locally: copy apps/admin/.env.example to apps/admin/.env.local and fill it in.",
    "On Cloudflare: `wrangler secret put SESSION_SECRET` from apps/admin.",
  ]);
}
