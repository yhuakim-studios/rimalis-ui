 ...
 * back. One extra hop per fifteen minutes for a signed-in shopper.
 *
 ...
 * harmless duplicate — it signs the shopper out everywhere.
 *
 ...
 *    `/account` link is a second concurrent render, and it is the most likely
 ...
 *    is cleared and the shopper is sent to sign in with an explanation, because
...
/** Never cached. A cached refresh would hand one shopper's tokens to another. */
...
export async function GET(request: NextRequest) {
  ...
  if (!result.ok) {
    ...
    // cart for a problem that fixed itself.
    //
