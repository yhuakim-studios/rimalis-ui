 ...
 * untampered-with.
 *
 * ## `SameSite=Lax`, NOT `Strict` — and this one is load-bearing
 *
 * `.env.example` says Strict, written before there was a payment flow. Strict is
 * wrong here and would break checkout in a way that is very hard to read from
 * the symptom:
 *
 *   Paystack returns the shopper to `/checkout/callback` as a **cross-site
 *   top-level navigation**. A Strict cookie is not sent on those. The callback
 *   page would therefore render for a signed-OUT visitor — someone who has just
 *   been charged, looking at a page that offers them a login form. The order is
 *   fine, the payment is fine, and the receipt is unreachable.
 *
 * Lax sends the cookie on exactly that navigation (top-level, GET) and still
 * withholds it from cross-site POSTs and subresource requests, which is the CSRF
 * surface that mattered. Server Actions carry Next's own origin check on top.
 *
...
export const SESSION_COOKIE = "rimalis_session";
 ...
 * produces a visitor the app believes is signed in, whose every request 401s.
 */
 ...
 * page the shopper had every right to see. Sixty seconds comfortably covers a
 ...
 *
 * The user summary is denormalised in on purpose: rendering "Hi, Ada" in the
 * header must not cost a `GET /users/me` on every request, and the fields it
 ...
 * it is what every visitor holds the moment `SESSION_SECRET` is rotated, and
 ...
 * site-wide 500 for everyone holding an old cookie.
 */
